...
-- Entrevistas de la semana pasada (completadas con resultados)
...
-- Actualizar entrevistas completadas con resultados (después de insertar)
UPDATE interviews SET 
    result = 'POSITIVE',
    score = 8.5,
    recommendations = 'Estudiante con excelente potencial académico. Se recomienda admisión.',
    completed_at = updated_at
WHERE id = (SELECT id FROM interviews WHERE status = 'COMPLETED' AND application_id = 6 LIMIT 1);
...
UPDATE interviews SET 
    ...
    completed_at = updated_at
WHERE id = (SELECT id FROM interviews WHERE status = 'COMPLETED' AND application_id = 7 LIMIT 1);
