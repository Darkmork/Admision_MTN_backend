...
public class PistonExecutorService {
    ...
    private String createExecutableCode(String userCode, String input) {
        ...
        executableCode.append("    \n");
        executableCode.append("    # Determinar si es un solo parámetro o múltiples\n");
        executableCode.append("    if input_str.startswith('[') and input_str.endswith(']'):\n");
        executableCode.append("        # Es una lista - un solo parámetro\n");
        executableCode.append("        param = parse_input(input_str)\n");
        executableCode.append("        result = ").append(functionName).append("(param)\n");
        executableCode.append("    elif input_str.startswith('(') and input_str.endswith(')'):\n");
        executableCode.append("        # Es una tupla - un solo parámetro\n");
        executableCode.append("        param = parse_input(input_str)\n");
        executableCode.append("        result = ").append(functionName).append("(param)\n");
        executableCode.append("    elif input_str.startswith('{') and input_str.endswith('}'):\n");
        executableCode.append("        # Es un diccionario - un solo parámetro\n");
        executableCode.append("        param = parse_input(input_str)\n");
        executableCode.append("        result = ").append(functionName).append("(param)\n");
        executableCode.append("    elif ',' in input_str and not any(input_str.startswith(x) for x in ['[', '(', '{', '\"', \"'\"]):\n");
        executableCode.append("        # Múltiples parámetros separados por comas\n");
        executableCode.append("        params_str = input_str.split(',')\n");
        executableCode.append("        params = [parse_input(p.strip()) for p in params_str]\n");
