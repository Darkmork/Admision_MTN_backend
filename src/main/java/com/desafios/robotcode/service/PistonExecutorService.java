...
public class PistonExecutorService {
    ...
    private String createExecutableCode(String userCode, String input) {
        ...
        executableCode.append("    # Intentar evaluar como expresión Python (listas, números, strings, etc.)\n");
        executableCode.append("    try:\n");
        executableCode.append("        # Usar ast.literal_eval para evaluar de manera segura\n");
        executableCode.append("        param = ast.literal_eval(input_str)\n");
        executableCode.append("        result = ").append(functionName).append("(param)\n");
        executableCode.append("    except (ValueError, SyntaxError):\n");
        executableCode.append("        # Si falla, intentar como múltiples parámetros separados por comas\n");
        executableCode.append("        if ',' in input_str and not (input_str.startswith('[') or input_str.startswith('(')):\n");
        executableCode.append("            # Múltiples parámetros separados por comas (no listas)\n");
        executableCode.append("            params_str = input_str.split(',')\n");
        executableCode.append("            params = []\n");
        executableCode.append("            for param_str in params_str:\n");
        executableCode.append("                param_str = param_str.strip()\n");
        executableCode.append("                try:\n");
        executableCode.append("                    param = ast.literal_eval(param_str)\n");
        executableCode.append("                except (ValueError, SyntaxError):\n");
        executableCode.append("                    # Si no se puede evaluar, usar como string\n");
        executableCode.append("                    param = param_str\n");
        ...
        executableCode.append("            # Un solo parámetro como string\n");
        executableCode.append("            result = ").append(functionName).append("(input_str)\n");
        executableCode.append("    \n");
        executableCode.append("    # Manejar output para que sea compatible con JSON\n");
        executableCode.append("    if result is None:\n");
        executableCode.append("        print('null')\n");
        executableCode.append("    elif isinstance(result, bool):\n");
        executableCode.append("        print('true' if result else 'false')\n");
        executableCode.append("    else:\n");
